...
class WCFM_WCSubscriptions {
    ...
    if( wcfm_is_subscription() ) {
    	...
    	// Order Type Indicator
			add_filter( 'wcfmmp_order_label_display', array( &$this, 'wcs_wcfm_orders_type' ), 50, 2 );
    }
  ...
  /**
	...
	function wcs_wcfm_orders_type( $order_label, $order_id ) {
		
		if ( wcs_order_contains_subscription( $order_id, 'renewal' ) ) {
			$order_label .= '<br /><span class="subscription_renewal_order wcfm-action-icon fa fa-history text_tip" data-tip="' . esc_attr__( 'Renewal Order', 'woocommerce-subscriptions' ) . '"></span>';
		} elseif ( wcs_order_contains_subscription( $order_id, 'resubscribe' ) ) {
			$order_label .= '<br /><span class="subscription_resubscribe_order wcfm-action-icon fa fa-history text_tip" data-tip="' . esc_attr__( 'Resubscribe Order', 'woocommerce-subscriptions' ) . '"></span>';
		} elseif ( wcs_order_contains_subscription( $order_id, 'parent' ) ) {
			$order_label .= '<br /><span class="subscription_parent_order wcfm-action-icon fa fa-dashcube text_tip" data-tip="' . esc_attr__( 'Parent Order', 'woocommerce-subscriptions' ) . '"></span>';
		}
