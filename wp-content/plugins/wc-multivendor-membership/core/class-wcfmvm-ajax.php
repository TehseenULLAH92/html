  ...
  function wcfmvm_email_verification_code() {
  	...
  	$user_email = $_POST['user_email'];
		...
		if( $user_email ) {
			...
			echo '{"status": true, "message": "' . sprintf( __( 'Verification code send to your email: %s.', 'wc-multivendor-membership' ), $user_email ) . '"}';
		} else {
  ...
  function wcfmvm_sms_verification_code() {
  	...
  	$user_phone = $_POST['user_phone'];
		...
		if( $user_phone ) {
			...
			echo '{"status": true, "message": "' . sprintf( __( 'Verification code send to your phone: %s.', 'wc-multivendor-membership' ), $user_phone ) . '"}';
		} else {
			$has_error = true;
		}
