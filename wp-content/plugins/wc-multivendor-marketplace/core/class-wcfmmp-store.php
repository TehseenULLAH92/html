...
class WCFMmp_Store {
	...
	public function get_store_tabs() {
		...
		if( !WCFM_Dependencies::wcfmu_plugin_active_check() || !apply_filters( 'wcfm_is_pref_vendor_followers', true ) || !apply_filters( 'wcfm_is_allow_store_followers', true ) ) {
		...
		if( !WCFM_Dependencies::wcfmu_plugin_active_check() || !apply_filters( 'wcfm_is_pref_vendor_followers', true ) || !apply_filters( 'wcfm_is_allow_store_followings', false ) ) {
			...
			$store_tabs['followings'] = __( 'Followings', 'wc-multivendor-marketplace' ) . ' (<span class="wcfm_followings_count">'.$followings.'</span>)';
		}
		...
		if( !apply_filters( 'wcfm_is_pref_policies', true ) || !$WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $this->id, 'vendor_policy' ) || !apply_filters( 'wcfm_is_allow_store_policy', true ) ) {
	...
	public function get_list_banner() {
		...
		return wp_get_attachment_url( $list_banner_id );
	}
	...
	public function get_avatar() {
		...
		if ( ! $avatar_id && ! empty( $this->data->user_email ) ) {
			return apply_filters( 'wcfmmp_store_default_logo', $WCFM->plugin_url . 'assets/images/avatar.png' ); //get_avatar_url( $this->data->user_email, 96 );
		}
	...
	public function get_store_policies() {
		$wcfm_policy_vendor_options = (array) get_user_meta( $this->get_id(), 'wcfm_policy_vendor_options', true );
		
		// WPML Support
		if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
			global $sitepress;
			$current_language = $sitepress->get_current_language();
			$wcfm_policy_vendor_options_wpml = (array) get_user_meta( $this->get_id(), 'wcfm_policy_vendor_options_'.$current_language, true );
			if( $wcfm_policy_vendor_options_wpml ) $wcfm_policy_vendor_options = $wcfm_policy_vendor_options_wpml;
		}
