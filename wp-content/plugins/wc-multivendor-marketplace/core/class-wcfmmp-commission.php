...
class WCFMmp_Commission {
				...
				if( $product_id ) {
					...
					if( $vendor_id ) {
						...
						if( $WCFMmp->wcfmmp_vendor->is_vendor_deduct_discount( $vendor_id ) ) {
							$commission_rule   = $WCFMmp->wcfmmp_product->wcfmmp_get_product_commission_rule( $product_id, $variation_id, $vendor_id, $line_item->get_total(), $line_item->get_quantity() );
							...
							$commission_rule   = $WCFMmp->wcfmmp_product->wcfmmp_get_product_commission_rule( $product_id, $variation_id, $vendor_id, $line_item->get_subtotal(), $line_item->get_quantity() );
						...
						$shipping_cost       = apply_filters( 'wcfmmmp_commission_shipping_cost', $shipping_cost, $vendor_shipping, $order_id, $vendor_id, $product_id, $commission_rule );
						...
						if( $get_shipping = $WCFMmp->wcfmmp_vendor->is_vendor_get_shipping( $vendor_id ) ) {
							...
							$total_commission += (float) $gross_shipping_tax;
						}
						...
						if( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $vendor_id ) ) {
							...
							if( $get_shipping ) {
								$grosse_total 		+= (float) $shipping_tax;
	...
	function wcfmmp_commission_order_item_delete( $item_id ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}wcfm_marketplace_refund_request WHERE item_id = %d", $item_id ) );
	}
