...
class WCFMmp_Ajax {
	...
	function wcfmmp_vendor_order_status_update( $order_id, $order_status ) {
		...
		if( $order_status == 'wc-shipped' ) {
			echo '{"status": false, "message": "' . __( 'This status not allowed, please go through Shipment Tracking.', 'wc-frontend-manager' ) . '"}';
			die;
		}
	...
	function wcfmmp_stores_list_search() {
		global $WCFM, $WCFMmp, $wpdb, $excludes;
		...
		add_filter( 'wcfm_exclude_vendors_list', function( $exclude_vendors ) {
			global $excludes;
			if( $excludes ) {
		...
		$stores = $WCFMmp->wcfmmp_vendor->wcfmmp_search_vendor_list( true, $offset, $length, $search_term, $search_category, $search_data, $has_product );
	...
	function wcfmmp_stores_list_map_markers() {
		...
		$stores = $WCFMmp->wcfmmp_vendor->wcfmmp_search_vendor_list( true, '', '', $search_term, $search_category, $search_data, $has_product );
		...
		if ( !empty( $stores )  ) {
			foreach ( $stores as $store_id => $store_name ) {
				...
				if( $store_lat && $store_lng ) {
					...
					$store_list_markers .= '{"name": "' . $store_name . '", "lat": "' . $store_lat . '", "lang": "' . $store_lng. '", "url": "' . $store_url . '", "address": "' . $store_address . '", "gravatar": "' . $gravatar . '"}';
