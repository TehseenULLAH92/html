...
class WCFMmp_Refund {
  ...
  function wcfmmp_refund_requests_form_html() {
	...
	public function wcfmmp_refund_orders_actions( $actions, $vendor_id, $order, $the_order ) {
  	...
  	if( !apply_filters( 'wcfm_is_allow_refund_requests', true ) ) return $actions;
		if( !apply_filters( 'wcfm_is_allow_paid_order_refund', false ) && ($order->withdraw_status != 'pending') ) return $actions;
		...
		$actions .= '<a class="wcfmmp_order_refund_request wcfm-action-icon" href="#" data-item="' . $order->item_id . '" data-commission="' . $order->ID . '" data-order="' . $order->order_id . '"><span class="fa fa-retweet text_tip" data-tip="' . esc_attr__( 'Refund Request', 'wc-multivendor-marketplace' ) . '"></span></a>';
  ...
  public function wcfmmp_refund_processed( $vendor_id, $order_id, $commission_id, $item_id, $refund_reason, $refunded_amount = 1, $refund_request = 'full', $refund_status = 'requested' ) {
	 ...
	 */
	public function wcfmmp_refund_status_update_by_refund( $refund_id, $status = 'completed' ) {
		global $WCFM, $WCFMmp, $wpdb;
		...
		if( $status == 'completed' ) {
				...
				foreach( $refund_infos as $refund_info ) {
					...
					if( $wc_refund_processed ) {
						...
						if( $commission_id ) {
								...
								foreach( $commission_infos as $commission_info ) {
									...
									if( $is_partially_refunded ) {
										if( $WCFMmp->wcfmmp_vendor->is_vendor_deduct_discount( $vendor_id ) ) {
											$item_total        = (float) $commission_info->item_total - ( $c_refunded_amount + (float) $commission_info->refunded_amount );
										} else {
											$item_total        = (float) $commission_info->item_sub_total - ( $c_refunded_amount + (float) $commission_info->refunded_amount );
										}
							...
							// Vendor Notification
							if( $vendor_id ) {
								$wcfm_messages = sprintf( __( 'Your Refund Request approved for Order <b>%s</b>.', 'wc-multivendor-marketplace' ), '<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url( $order_id ) . '">#' . $order_id . '</a>' );
							...
							do_action( 'wcfmmp_commission_refund_status_completed', $refund_id, $commission_id );
						}
					
						// Refund Status Updated
						$wpdb->update("{$wpdb->prefix}wcfm_marketplace_refund_request", array('refund_status' => $status, 'refunded_amount' => $c_refunded_amount, 'refund_paid_date' => date('Y-m-d H:i:s', time())), array('ID' => $refund_id), array('%s', '%s', '%s'), array('%d'));
						...
						do_action( 'wcfmmp_refund_status_completed', $refund_id );
			...
			if( !empty( $refund_infos ) ) {
				foreach( $refund_infos as $refund_info ) {
					...
					// Refund Status Updated
					$wpdb->update("{$wpdb->prefix}wcfm_marketplace_refund_request", array('refund_status' => $status, 'refund_paid_date' => date('Y-m-d H:i:s', time())), array('ID' => $refund_id), array('%s', '%s'), array('%d'));
