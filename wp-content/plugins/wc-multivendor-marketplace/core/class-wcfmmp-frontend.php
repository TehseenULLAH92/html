...
class WCFMmp_Frontend {
	...
	public function __construct() {
		...
		if( function_exists( 'martfury_is_vendor_page' ) ) {
			...
			// Store Page Toolbar
			add_action( 'woocommerce_before_shop_loop', array( $this, 'wcfmmp_store_toolbar' ), 20 );
		}
	...
	/**
	 * Martfury Store Page toolbar
	 */
	function wcfmmp_store_toolbar() {
		if( function_exists( 'wcfm_is_store_page' ) && wcfm_is_store_page() ) {
			
			$catalog_layout   = martfury_get_catalog_layout();
			$els = martfury_get_option( 'catalog_toolbar_els_' . $catalog_layout );
	
			$elements = apply_filters( 'martfury_catalog_toolbar_elements', $els );
			...
			if ( in_array( 'view', $elements ) ) {
				$list_current = $shop_view == 'list' ? 'current' : '';
				$grid_current = $shop_view == 'grid' ? 'current' : '';
				$output[]     = sprintf(
					'<div class="shop-view">' .
					'<span>%s</span>' .
			...
			if ( in_array( 'filter', $elements ) ) {
				$output[] = sprintf( '<a href="#" class="mf-filter-mobile" id="mf-filter-mobile"><i class="icon-equalizer"></i>%s</a>', esc_html__( 'Filter', 'martfury' ) );
			}
	...
	function wcfmmp_scripts() {
		...
		if( wcfmmp_is_stores_list_page() || wcfmmp_is_stores_map_page() ) {
			...
			wp_localize_script( 'wcfmmp_store_list_js', 'wcfmmp_store_list_options', array( 'is_geolocate' => apply_filters( 'wcfmmp_is_allow_store_list_by_user_location', true ) ) );
		}
