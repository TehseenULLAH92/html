...
function wcfmmp_get_store_url( $user_id ) {
	...
	$wcfm_store_url = get_option( 'wcfm_store_url', 'store' );
	return trailingslashit( trailingslashit( home_url( $wcfm_store_url ) ) . $user_nicename );
}
...
if( WCFMmp_Dependencies::wcfm_plugin_active_check() && WCFM_Dependencies::wcfm_geo_my_wp_plugin_active_check() && apply_filters( 'wcfm_is_allow_product_location_display', false ) ) {
...
function wcfmmp_pos_rest_request_before_callbacks( $response, $handler, $request ) {
	...
	if( $wc_api_handler == 'WC_REST_Products_Controller' ) {
		if( is_user_logged_in() && wcfm_is_vendor() ) {
			add_action( 'pre_get_posts', 'wcfmmp_pos_pre_get_posts' );
		}
	}
	return $response;
}
